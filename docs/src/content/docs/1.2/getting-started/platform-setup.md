...
macOS uses the same `webview_flutter_wkwebview` package as iOS, but not every UIKit-backed WebKit property has a macOS equivalent. See [iOS and macOS](/webview_all/1.2/platforms/ios-macos/) for the exact limits.
