...
import 'package:webview_all_android/src/android_webkit.g.dart'
    as android_webview;
import 'package:webview_all_android/src/android_webkit_constants.dart';
...
void main() {
  ...
  test('isWebViewFeatureSupported for Web Authentication', () async {
    String? captured;
    ...
    expect(WebViewFeatureConstants.webAuthentication, captured);
    expect(expectedIsWebViewFeatureEnabled, result);
  });
  ...
  test('setWebAuthenticationSupport maps every support level', () async {
    android_webview.WebSettings? capturedSettings;
    int? capturedSupport;
    ...
    for (final WebAuthenticationSupport support
        in WebAuthenticationSupport.values) {
      await controller.setWebAuthenticationSupport(support);

      expect(capturedSettings, mockSettings);
      expect(capturedSupport, support.value);
    }
  });

  test('setWebAuthenticationSupport propagates platform errors', () async {
    final UnsupportedError expectedError = UnsupportedError(
      'WEB_AUTHENTICATION is not supported',
    );
    final AndroidWebViewController controller = createControllerWithMocks(
      setWebAuthenticationSupport: (_, __) async => throw expectedError,
    );
