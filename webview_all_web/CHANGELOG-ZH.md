...
* Windows 新增开发者工具、浏览器快捷键和下载开关，支持创建时配置及运行时调整。
* 修复 Windows 端通过透明度动画隐藏后，再次显示时可能空白或画面停滞的问题。
* 修复 Web 关闭 JavaScript 时覆盖自定义 iframe sandbox 限制的问题。
* Linux 新增下载开关，可按 WebView 单独控制是否允许下载。
